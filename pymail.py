...
def loadmessages(servername,user,passwd,loadfrom = 1):
    ...
    try:
        ...
        server.quit()
    assert len(msgList) == (msgCount - loadfrom) + 1 #только одно сообщение
    return msgList
...
def showindex(msgList):
    ...
    for msgtext in msgList:
        ...
        count+=1
        print('%d:\t%d bytes' % (count,len(msghdrs)))
        for hdr in ('From','To','Date','Subject'):
            try:
                print('\t%-8s=>%s' % (hdr,msgtext[hdr]))
            except KeyError:
